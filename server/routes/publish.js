...
router.post('/:skillId/publish', async (req, res) => {
  try {
    const skill = db.getSkill(req.params.skillId);
    ...
    // 记录发布
    db.createPublishRecord(skill.id, channel.id);

    // 更新 skill 状态
    db.updateSkill(skill.id, { status: 'published' });
...
router.post('/:skillId/unpublish', async (req, res) => {
  try {
    ...
    // 获取所有已发布记录
    const records = db.listPublishRecords(req.params.skillId)
      .filter(r => r.status === 'published');

    for (const record of records) {
      const channel = db.getChannel(record.channel_id);
      if (channel) {
        const publisher = createChannel(channel.type, channel.config);
    ...
    db.unpublishRecords(req.params.skillId);
    db.updateSkill(req.params.skillId, { status: 'unpublished' });
...
// 获取 skill 的发布记录
router.get('/:skillId/records', (req, res) => {
  const records = db.listPublishRecords(req.params.skillId);
